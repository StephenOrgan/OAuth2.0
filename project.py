...
def fbconnect():
  ...
  # Exchange client token for long-lived server-side token with /GET /oauth/access_token?grant_type=fb_echange_token
  ...
  url = 'https://graph.facebook.com/me?access_token=%s&fields=name,id,email' % token
  h = httplib2.Http()
  result = h.request(url, 'GET')[1]

  # print "url sent for API acccess:%s" % url
...
def editMenuItem(restaurant_id, menu_id):
    ...
    if 'username' not in login_session:
      return redirect('/login')
    if reditedItem.user_id != login_session['user_id']:
